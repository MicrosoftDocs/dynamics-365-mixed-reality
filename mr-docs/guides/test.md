...
9. In the **PowerShell Interactive Login** dialog box, connect to the source instance by doing the following: 
    ...
    [!NOTE]
    If you don’t select an organization to connect to, the tool connects to the last one that was successful for both source and destination. If there’s only one instance in the tenant, PowerShell automatically connects to the default instance and you can skip to Step 11.<br><br>
 
