...
>[!IMPORTANT]
>On **April 1**, we will release a **mandatory** update to the Dynamics 365 Guides solution that will change how data is stored in Common Data Service. Specifically, we'll upgrade the applications to store data in Common Data Service entities instead of in a single JSON file. Your existing guides will be migrated to the new entity structure and will continue to work as expected. On April 1, you'll need to update the PC app, HoloLens app, and Common Data Service solution to continue using Dynamics 365 Guides. Please work with your admin to update the solution and apps.
