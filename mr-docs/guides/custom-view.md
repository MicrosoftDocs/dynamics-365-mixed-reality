...
3. In the **New entity** pane on the right side of the screen, enter a display name (for example, **Custom Tab**). 
    ...
    >[!IMPORTANT]
    >The display name must be singular. If the display name is plural, the tab will not show properly on HoloLens.
...
6. In the **Field properties** pane on the right side of the screen:
   ...
   2. In the **Data type** field, select **Date and Time**. 
       ...
       ![Date and Time fields](media/custom-view-date-time-field.PNG "Date and Time fields")
...
7. Select **Add field** again, and then enter the following values for this field:
   ...
   3. In the **Related entity** field, enter **Guide**. 
       ...
       ![Lookup fields filled in](media/custom-view-lookup-field.PNG "Lookup fields filled in")
...
2. Select **Views**, and then select **Add view**.
    ...
    ![Add view](media/custom-view-create-view.PNG "Add view")

3. In the **Create a view** dialog box, enter a name for the view. This name will be displayed in the **HoloLens** tab.

    ![Enter view name](media/custom-view-view-name.PNG "Enter view name")
...
8. Copy and save the saved query record ID. This record ID appears at the end of the URL for the window.
    ...
    ![Saved query ID at end of url](media/custom-view-queryid.PNG "Saved query ID at end of url")
...
4. In the **New Guide View Configuration** screen, enter a name that will help you remember what this configuration record is for. This is not used for the HoloLens tab name. 
    ...
    ![Name field](media/custom-view-name-guides-configuration.PNG "Name field")
...
5. In the **Saved Query ID** field, enter the record ID you saved in step 8 of the previous procedure. This value isn't validated, so make sure to copy it carefully to ensure that it's the correct record ID.
    ...
    ![Saved Query ID field](media/custom-view-saved-queryid.PNG "Saved Query ID field")
...
2. In the left pane, select **Apps**, select the **More Commands** (...) button next to **Guides**, and then select **Edit**.
    ...
    ![Edit button](media/custom-view-guides-edit-button.PNG "Edit button")

3. In the App Designer, select the **Edit** button next to **Site Map**.

    ![Site map Edit button](media/custom-view-sitemap-edit.PNG "Site map Edit button")
...
6. Select the heading for the new group, select **Add**, and then select **Subarea**.
    ...
    ![Add subarea](media/custom-view-add-subarea.PNG "Add subarea")
...
7. In the **Properties** tab on the right side of the screen, in the **Entity** field, select the name of the entity you created in [Step 1: Create a new entity in Common Data Service](#step-1-create-a-new-entity-in-common-data-service) to link the subarea. 
    ...
    ![Link subarea](media/custom-view-link-subarea.PNG "Link subarea")
...
10. Scroll down to the **Forms** item for your entity, and then select it.
    ...
    ![Select form for entity](media/custom-view-select-forms.PNG "Select form for entity")
...
13. Go to [https://powerapps.microsoft.com](https://powerapps.microsoft.com). In the left pane, you'll see your new group and view at the bottom. 
    ...
    ![New view in left pane](media/custom-view-custom-tab-configuration.PNG "New view in left pane")

14. Select the view to open it, and then in the **Guide** field, select a guide to add to the list.

    ![Select instance](media/custom-view-select-list.PNG "Select instance")
    
    >[!IMPORTANT]
    >When you add a guide to the tab list, you must specify a date. This date must be the current or future date. Otherwise, the guide will not appear in the custom tab. 
