...
Autosave checks for changes every four seconds. You must be online to use [!include[pn-dyn-365-guides](../includes/pn-dyn-365-guides.md)]
...
- To provide overall context, add an Overview task at the start of the guide. This task includes just one step that describes what the guide is about. This is also a good place to list all the tasks that the guide covers. 
...
The following image shows an example of a filled-out **Outline** page.

![Filled-out Outline page from Pylon wiring guide](media/finished-outline-page.png "Filled-out Outline page from the Pylon wiring guide")
...
1. Select any step on the **Outline** page, or select **Step** in the left pane. 

   ![Step command in the left pane](media/left-nav-step.PNG "Step command in the left pane")
...
- It's easy to add another step from the Step card page. Just select **Add new step** in the upper-right corner of the page; there's no need to go back to the **Outline** page. 

  ![Add new step button](media/add-new-step-button.PNG "Add new step button")

- Don't be afraid to add lots of steps, but keep them short. Instruction text is limited to 280 characters per step to help keep the text short.
...
![List of objects in the 3D toolkit](media/3D-toolkit-objects.png "List of objects in the 3D toolkit")
...
3.	Drag the object you want from the list to a **3D parts** box below the Step card.
    ...
    ![Drag an object from the 3D toolkit](media/3d-part.PNG "Drag an object from the 3D toolkit")
...
- Adjust the size of the pointer, but never make it smaller than 1 cm. If you do so, it can cause errors.

## Keyboard shortcuts

You can use any of the following keyboard shortcuts in the PC app:
...
|Ctrl+Left arrow key<br>Ctrl+Right arrow key|Move to the previous or next task or step, depending on what's selected|
|Ctrl+Up arrow key<br>Ctrl+Down arrow key|Move to above or below the task or step, depending on what's selected|
|Windows logo key+Down arrow key|Minimize the current window|
|Windows logo key+Up arrow key|Maximize the current window|
...
To turn telemetry data off for the PC app:
...
Don't worry if your guide isn't perfect before you switch to [!include[pn-hololens](../includes/pn-hololens.md)] authoring. You can always go back to the PC app to edit and restructure your guide. In fact, we've found it's generally a good idea to do a quick test of the flow on [!include[pn-hololens](../includes/pn-hololens.md)], and then go back to the PC app to make changes before placing all your holograms in [!include[pn-hololens](../includes/pn-hololens.md)].
