...
1. The dashboard leverages the phonecall entity type to generate analytics. End users must [select the right Dynamics environment](./asset-capture-add-users#selecting-the-right-environment-from-the-client-app) from their device to send the [phonecall](https://docs.microsoft.com/dynamics365/customer-engagement/web-api/phonecall) record to the Common Data Service. Only after they have selected the correct environment, and for those users only, is the phonecall data available in the calls dashboard.
