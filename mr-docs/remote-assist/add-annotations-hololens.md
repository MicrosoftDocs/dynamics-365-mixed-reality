...
Enabling the PV Camera Render setting will improve the alignment accuracy of annotations as seen by Microsoft Teams users in a Remote Assist session.  This will ensure that annotations placed by the HoloLens 2 Remote Assist environment are represented accurately for the remote expert.  When this setting is turned off, the Microsoft Teams user(s) may notice the annotations placed in the HoloLens 2 user environment to be slightly out of place.

![Graphic showing the PV Camera Render Setting](media/RAHL_PVSetting.png).

in order to accommodate the additional system resources required for utilizing PV Camera Render, enabling PV Camera Render limits the maximum outgoing video resolution of a Remote Assist session to HD quality (e.g. 720p).  Turning off the PV Camera Render setting will allow the maximum outgoing video resolution to be 1080p.
