...
5. Drag and drop your model folder into this location. It will automatically show up in Product Visualize. Add models using the following folder structure:
   ...
   3. Products can have multiple models inside of them. Create a new folder and model file for each model that you want to add.
      ...
      Example: (Italicized folders are automatically generated by Dynamics 365. Add the files and folders marked as bold.)
     ...
     > [!NOTE]
     > SharePoint folders are created when you first visit the documents folder through a Dynamics 365 app. If the folder for a Product doesn't exist yet, return to the Sales Hub and open the Product's document location from there. After it's created, you can access it through SharePoint.
 ...
 6. Optional: SharePoint will automatically provide a thumbnail for Product Visualize. However, if you want to add an optional custom thumbnail to a model, do the following:
    ...
    2. Add a PNG image (with the same name as the model folder) and the model file into the **Thumbnails** folder.
       ...
       Example: (Italicised folders are automatically generated by Dynamics 365. Add the files and folders marked as bold.)
