...
Texture coordinates (also called UVs) are pairs of numbers (U and V) stored in the geometry of a model. These numbers represent how a 2D image can be mapped to a 3D surface and enable the ability for surface properties to be defined as an image. These images can be simple decals or can be complex components of how a materials is created – roughness, metallic, and so on. Generating UV positions for a mesh can be done manually, semi-automatically, or automatically. The AutoUV surface operator is one tool in Houdini capable of generating UVs. It uses a few automatic methods during the creation of UVs.

### To bake the texture

1. Add an AutoUV node to the scene by right-clicking the **Geometry** pane and selecting **Labs** > **UV** > **Labs Auto UV**.
